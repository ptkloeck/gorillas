...
public class City extends ImageDestructibleGameObject {
    ...
    /**
     * The upper left corners of the buildings relative to the city position. In this case the
     * y-axis is pointing up.
     */
    private ArrayList<Vector2> buildingCoordinates;

    /**
     * The position of the left gorilla relative to the city position. In this case the y-axis is
     * pointing up.
     */
    private Vector2 leftGorillaCoordinate;
    /**
     * The position of the right gorilla relative to the city position. In this case the y-axis is
     * pointing up.
     */
    ...
    public City(Vector2 position, Pixmap original, Pixmap destructionPattern, String destructionPatternID,
    ...
    private void randChooseGorillaCoordinates() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (buildingWidth >= gorillaWidth) {
                ...
                leftPositions.add(new Vector2(buildingStartX + buildingWidth / 2 - gorillaWidth / 2,
                        buildingY));
            }
        }

        // choose random coordinate out of the possible ones
        leftGorillaCoordinate = (Vector2) Utils.randomChooseFrom(leftPositions.toArray());

        // list of possible coordinates for the right gorilla
        ArrayList<Vector2> rightPositions = new ArrayList<Vector2>();

        for (int i = buildingCoordinates.size() - 1; i > buildingCoordinates.size() - 4; i--) {
            ...
            if (buildingWidth >= gorillaWidth) {
                float buildingY = buildingCoordinates.get(i).y;
                rightPositions.add(new Vector2(buildingStartX + buildingWidth / 2 - gorillaWidth / 2,
                        buildingY));
            }
        }

        // choose random coordinate out of the possible ones
        rightGorillaCoordinate = (Vector2) Utils.randomChooseFrom(rightPositions.toArray());
    }
