...
public class GorillaFactory {
	...
	public static Gorilla createGorilla(Vector2 position, Direction direction) {
		...
		if (direction == Direction.LEFT_TO_RIGHT) {
			...
			shotStartX = position.x + original.getWidth()
					- Assets.banana.getRegionWidth();
		}
		Vector2 shotStartPosition = new Vector2(shotStartX, position.y
				- Assets.banana.getRegionWidth() - SHOT_DISTORTION_Y);
		Gorilla gorilla = new Gorilla(position, original, destructionPattern,
				City.gorillaDestrPatternID, direction, shotStartPosition);
