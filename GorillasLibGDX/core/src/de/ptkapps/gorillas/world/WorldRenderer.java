...
public class WorldRenderer {
    ...
    public WorldRenderer(SpriteBatch batch, World world) {
        ...
        viewport = new ExtendViewport(Gorillas.worldWidth, Gorillas.worldHeight, camera);
    ...
    private void renderOldParameterization(Parameterization param) {
        ...
        if (arrowStartPos != null && arrowEndPos != null) {
            ...
            Color color = GUIConstants.BACKGROUND_COLOR.cpy().lerp(OLD_CONTROL_ARROW_COLOR, t);
            drawArrow(arrowStartPos, arrowEndPos, CONTROL_ARROW_WIDTH, CONTROL_ARROW_HEADLINE_LENGTH, color);
        }
    }
    ...
    private void renderNewParameterization(Parameterization param) {
        ...
        if (param.getDirection().equals(Direction.LEFT_TO_RIGHT)) {
            arrowStartPos = world.gorilla1.getShotStartPosition();
        } else {
            arrowStartPos = world.gorilla2.getShotStartPosition();
        }

        Vector2 arrowEndPos = arrowStartPos.cpy().add(paramStartPos.cpy().sub(paramEndPos.cpy()));

        param.rememberParameterization(arrowStartPos, arrowEndPos);

        drawArrow(arrowStartPos, arrowEndPos, CONTROL_ARROW_WIDTH, CONTROL_ARROW_HEADLINE_LENGTH,
                CONTROL_ARROW_COLOR);
    }

    /**
     * draws an arrow, which starts in <code>startPos</code> and ends in <code>endPos</code>. Both
     * the body of the arrow and the two headlines have a width of <code>arrowWidth</code>. The two
     * headlines both have a length of <code>headlineLength</code>.
     * 
     * The <code>shapeRenderer</code>, which is used expects the y-axis to point up.
     * 
     * @param startPos
     * @param endPos
     * @param arrowWidth
     ...
     * @param color
     */
    private void drawArrow(Vector2 startPos, Vector2 endPos, float arrowWidth, float headLineLength,
            Color color) {
        ...
        shapeRenderer.rect(-arrowWidth, length + arrowWidth / 2, arrowWidth, 0, arrowWidth, headLineLength,
                1, 1, 135);
        shapeRenderer.rect(0, length + arrowWidth / 2, 0, 0, arrowWidth, headLineLength, 1, 1, 225);
    }
    ...
    private void renderWind() {
        ...
        if (wind != 0) {
            ...
            if (windLine < 0) {
                ...
                shapeRenderer.translate(startX + arrowWidth / 2f + halfArrowHeight, startY + arrowHeight, 0);
                ...
                shapeRenderer.translate(-1 * (arrowWidth / 2f + halfArrowHeight), 0, 0);
            }
            ...
            shapeRenderer.rect(startX + arrowWidth + halfArrowHeight, startY + halfArrowHeight, 0, 0,
                    headLineWidth, arrowHeight, 1, 1, 135);
            // draw upper head line
            shapeRenderer.rect(startX + arrowWidth + halfArrowHeight, startY - halfArrowHeight, 0,
                    arrowHeight, headLineWidth, arrowHeight, 1, 1, 225);
        }
    }
    ...
    private void renderGorilla(Gorilla gorilla) {
        ...
        if (gorilla.state == Gorilla.GORILLA_STATE_THROWING) {
            ...
            if (gorilla.getDirection() == Direction.LEFT_TO_RIGHT) {
                gorillaKeyFrame = Assets.leftThrowingGorilla.getKeyFrame(gorilla.stateTime, false);
            } else {
                gorillaKeyFrame = Assets.rightThrowingGorilla.getKeyFrame(gorilla.stateTime, false);
            }

            batch.draw(gorillaKeyFrame, (int) gorilla.getPosition().x, (int) gorilla.getPosition().y);

        } else if (gorilla.state == Gorilla.GORILLA_STATE_CHEERING) {

            gorillaKeyFrame = Assets.cheeringGorilla.getKeyFrame(gorilla.stateTime, false);

            batch.draw(gorillaKeyFrame, (int) gorilla.getPosition().x, (int) gorilla.getPosition().y);

        } else if (gorilla.state == Gorilla.GORILLA_STATE_DESTRUCTED) {

            batch.draw(Assets.gorillaDestructed, (int) gorilla.getPosition().x, (int) gorilla.getPosition().y);
            ...
            batch.draw(gorillaTexture, (int) gorilla.getPosition().x, (int) gorilla.getPosition().y);
        }
    }
    ...
    private void renderShot() {
        ...
        if (world.currentShot != null) {
            batch.draw(Assets.banana, world.currentShot.getPosition().x, world.currentShot.getPosition().y,
                    shot.getWidth() / 2, shot.getHeight() / 2, shot.getWidth(), shot.getHeight(), 1, 1,
                    shot.getRotation() * 360);
        }
    }
    ...
    private void renderCity() {
        ...
        if (city.state == City.CITY_STATE_BEEING_DESTRUCTED) {
            ...
            batch.draw(Assets.cityDestruction.getKeyFrame(city.stateTime),
                    hitPosition.x - explosionWidth / 2, hitPosition.y - explosionHeight / 2);
        }
    }
