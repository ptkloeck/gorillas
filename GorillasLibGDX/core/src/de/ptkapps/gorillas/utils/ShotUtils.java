...
public class ShotUtils {
	...
	/**
	 * Predicts how far (see the description of the return value) a shot will
	 * fly until it hits the screen bottom.<br>
	 * <br>
	 * At the time the shot reaches the screen bottom the y value of the shot
	 * equals the frame height. Thus, following equation has to be solved to
	 * determine the time the shot needs to reach the screen bottom (see
	 ...
	 * 
	 * @return the x value of the shot at the time the shot reaches the y level
	...
	/**
	 * This method determines the next position of a <em>Shot</em> object, based
	 * on the time passed, the initial launching angle, launching speed and the
	 ...
	 * <br>
	 * Note that the upper formulas have to be slightly adjusted depending on
	 * whether the shot flies from right to left or from left to right (upper
	 * formulas hold for the case left to right; wind and v_x * t have to be
	 * subtracted in the case right to left).
	 * 
	 * @param shot
	 *            The <em>Shot</em> entity.
	 * @param delta
	 *            The amount of time passed, used to determine how far the
	 *            object has to move, in milliseconds. This value will be scaled
	 *            by factor 1/100 to provide a more realistic flying curve.
	 */
	public static Vector2 determineNextPosition(Shot shot, float delta) {
		float g = Options.getInstance().getGravity();

		// update the current time
		// float t = shot.getTimeElapsed() + delta * Gorillas.worldWidth / 100;
	...
	/**
	 * calculates the velocity to hit target (x, y) given the start position
	 * (x0, y0) the current wind value and the starting angle. Beware: This
	 * function only delivers the correct result for a shot from right to left! <br>
	 * <br>
	 * Starting with the shot position formulas for the case from right to
	 * left... <br>
	 * (1) x = x_0 - (cos(apha) * v * t) - wind<br>
	 * (2) y = y_0 - (sin(alpha) * v * t) + (1/2 * g * t^2) <br>
	 * wind = 1/2 * (windValue / 5) * t^2 <br>
	 * <br>
	 * ... the following formulas for v can be obtained from (1) and (2) by each
	 * activating towards v: <br>
	 * (3) v = y - y_0 - 1/2 * g * t^2 / (-sin(alpha) * t)<br>
	 * (4) v = x - x_0 + 1/2 * wind / 5 * t^2 / (cos(alpha) * t) <br>
	 * <br>
	 * This leads to:<br>
	 * y - y_0 - 1/2 * g * t^2 / (-sin(alpha) * t) = x - x_0 + 1/2 * wind / 5 *
	 * t^2 / (cos(alpha) * t).<br>
	 * <br>
	 * Multiplying with (-sin(alpha) * t) and then with (cos(alpha) *t) leads
	 * to:<br>
	 * (y - y_0) * cos(alpha) + 1/2 * g * cos(alpha) * t^2 = -(x - x_0) *
	 * sin(alpha) + 1/2 * sin(alpha) * wind / 5 * t^2<br>
	 * <br>
	 * This can be rewritten to:<br>
	 * (1/2 * g * cos(alpha) + 1/2 * sin(alpha) * wind/5) * t^2 = -(x * x_0) *
	 * sin(alpha) + (y - y_0) * cos(alpha)<br>
	 * <br>
	 * This formula can be activated towards t, and then t can be used to
	 * calculate v using either (3) or (4).<br>
	 * <br>
	 * 
	 * @param x0
	 ...
	 */
	public static float determineNeededVelocity(float x0, float y0, float x,
			float y, float wind, float angle) {
