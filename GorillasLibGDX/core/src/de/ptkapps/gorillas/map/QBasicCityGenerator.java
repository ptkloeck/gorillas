...
public class QBasicCityGenerator {
    ...
    /**
     * Increment value for the next building height. Buildings are added to the city one after
     * another from left to right. Based on a randomly selected slope type the basic height of the
     * next building is calulated by adding or subtracting one or two times this summand to the
     * height of the current building.
     */
    private int heightIncrement;

    /**
     * The basic height of a building based on the slope type is randomized by adding this summand.
     */
    private int randHeightDifference;

    /**
     * The basic height of the first building in case of a slope type with a small first building.
     */
    private int bottomSlopeStart;

    /**
     * The basic height of the first building in case of a slope type with a heigh first building.
     */
    private int topSlopeStart;

    /**
     * Minimum building width equals default building width plus 2. The width of a building is
     * calculated by adding a random summand from zero to default building width (inclusive) to the
    ...
    /**
     * the top left corners of the builings relative to the city position (y-axis is pointing up)
     */
    private ArrayList<Vector2> buildingCoordinates;

    /**
     * the pixmap of the city, which can be used to create a texture and draw the city.
     */
    private Pixmap city;

    public QBasicCityGenerator() {
        defaultBuildingWidth = Assets.gorilla.getRegionWidth() + BUILD_GORILLA_WIDTH_ADD;
    }
     ...
     * @param yOffsetCity
     *            The y offset of the resulting city (y-axis pointing up). This value is only used
     *            to assign a position to the resulting city. The city will have a position of (0,
    ...
    public City generateCity(int cityWidth, int cityHeight, int yOffsetCity) {
        ...
        return CityFactory.createCity(new Vector2(0, yOffsetCity), city, buildingCoordinates);
    }

    public City generateCity(int cityWidth, int cityHeight, int yOffsetCity,
            ArrayList<Vector2> buildingCoordinates, Vector2 leftGorillaCoordinate,
            Vector2 rightGorillaCoordinate) {
    ...
    /**
     * initializes the values used for randomly creating a new city based on the given
     * <code>cityHeight</code> and <code>cityWidth<code>
     */
    private void initBuildValues() {
        ...
        int maxNumberBuildings = (int) Math.ceil(cityWidth / (float) defaultBuildingWidth);

        heightIncrement = (int) Math.floor(topSlopeStart / (float) maxNumberBuildings);
    }
    ...
    private void generateSkyline() {
        ...
        int x = 2;
        do {
            currentBasicHeight = calcNextBasicHeight(x, slopeType, currentBasicHeight, cityWidth);
            ...
            // small
            int buildingHeight = random.nextInt(randHeightDifference + 1) + 1 + currentBasicHeight;
    ...
    private void drawSkyline() {
        ...
        city.fillRectangle(cityWidth / 2 - (int) Gorillas.scoreLabelWidth / 2, cityHeight
                - (int) Gorillas.normalLabelHeight - 5, (int) Gorillas.scoreLabelWidth,
                (int) Gorillas.normalLabelHeight);
    }

    /**
     * determines the basic height for the next building according to the slopeType
     * 
     * @param x
     ...
     *            the basic height of the building left to the building, whose basic height should
     *            be returned
     * @param cityWidth
     ...
     */
    private int calcNextBasicHeight(int x, int slopeType, int currentBasicHeight, int cityWidth) {
    ...
    /**
     * determines the basic height for the first building according to the slopeType
     * 
     * @param slopeType
     *            the slopeType of the city
    ...
    /**
     * draws a single building a the specified x position on the graphic with the width specified in
     * buildingWidth and the height specified in buildingHeigt
     * 
     * @param pixmap
     ...
     */
    private void drawBuilding(Pixmap pixmap, int x, int buildingWidth, int buildingHeight) {

        int argb = chooseBuildingColor();

        // Draw the building
        pixmap.setColor(new Color(argb));
        pixmap.fillRectangle(x, cityHeight - buildingHeight, buildingWidth, buildingHeight);
        ...
        do {
            for (int i = buildingHeight - 3; i >= 7; i -= WINDOW_SPACING_VERTICAL) {
                ...
                pixmap.setColor(new Color(argb));
                pixmap.fillRectangle(c, cityHeight - i, WINDOW_WIDTH, WINDOW_HEIGHT);
            }
    ...
    /**
     * randomly chooses a building color out of the three QBasic colors light grey, dark red and
     * turquoise
     * 
    ...
    /**
     * randomly chooses a window color out of the two QBasic colors dark grey and yellow
     * 
     * @return the window color encoded as Color.ARGB value
     */
