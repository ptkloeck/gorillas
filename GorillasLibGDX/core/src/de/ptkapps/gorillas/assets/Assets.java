...
public class Assets {
    ...
    /**
     * creates TextureRegions and Animations from the Textures loaded by the given AssetManager and
     * makes references to the Sounds and Pixmaps. ATTENTION: the manager has to have finished
     * loading the assets.
     * 
     * @param manager
     *            the AssetManager, which has finished loading the assets of the game
     */
    public static void finishSetup(AssetManager manager) {

        bananaCityDestruction = manager.get(pathPrefix + DESTRUCTION_PATH, Pixmap.class);
        gorillaCityDestruction = manager.get(pathPrefix + GORILLA_DESTRUCTION_PATH, Pixmap.class);
        originalGorilla = manager.get(pathPrefix + GORILLA_PATH, Pixmap.class);

        gorilla = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(GORILLA);
        gorillaLeftArmUp = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(
                GORILLA_LEFT_UP);
        gorillaRightArmUp = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(
                GORILLA_RIGHT_UP);

        gorillaDestructed = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(
                GORILLA_DESTRUCTED);

        cheeringGorilla = new Animation(0.2f, gorillaLeftArmUp, gorillaRightArmUp, gorillaLeftArmUp,
                gorillaRightArmUp, gorillaLeftArmUp, gorillaRightArmUp, gorillaLeftArmUp, gorillaRightArmUp,
                gorillaLeftArmUp, gorillaRightArmUp, gorillaLeftArmUp, gorillaRightArmUp);
        ...
        //cityDestruction1 = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(EXPLOSION_1);
        //cityDestruction2 = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(EXPLOSION_2);
        cityDestruction1 = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion("explosion");
        cityDestruction2 = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion("explosion");
        cityDestruction = new Animation(0.2f, cityDestruction1, cityDestruction2);

        banana = new TextureRegion(manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(
                BANANA));

        sunSmiling = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(SUN_SMILING);
        sunAstonished = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(SUN_ASTONISHED);

        britishFlag = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(UNION_JACK);
        germanFlag = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(GERMAN);

        soundOn = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(SOUND_ON);
        soundOff = manager.get(pathPrefix + "items.pack", TextureAtlas.class).findRegion(SOUND_OFF);
